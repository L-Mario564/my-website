<script lang="ts">
  ...
  export let tournament:
    | Simplify<TournamentPlayed & { type: 'played' }>
    | Simplify<TournamentStaffed & { type: 'staffed' }>;
...
{#if $showWebsiteScreenshots && tournament.type === 'staffed'}
  <WebsiteSlideshow
    website={websites.find((website) => website.id === tournament.websiteId) ?? websites[0]}
    close={onSlideshowClose}
  />
{/if}
<div
  role="presentation"
  class="relative w-full xs:w-[21.75rem] h-max opacity-0"
  on:mouseenter={showDetails.toTrue}
  on:mouseleave={showDetails.toFalse}
  bind:this={tournamentContainer}
>
  <button
  ...
  <div class="absolute w-full h-full top-0 left-0 z-[12] p-4">
    <div class="relative h-[28px]">
      {#if $showTeams && tournament.type === 'played'}
        <strong
          class="text-white font-bold text-xl absolute top-0 left-0"
          transition:fade={{ duration: 150 }}>{tournament.team?.name}</strong
        >
      {:else}
        <strong
          class={`text-white font-bold text-xl absolute top-0 left-0${tournament.name.length > 28 ? ' tracking-tight' : ''}`}
          transition:fade={{ duration: 150 }}>{tournament.name}</strong
        >
      {/if}
    </div>
    <div
    ...
    <div class="relative">
      {#if $showTeams && tournament.type === 'played'}
        <div
          ...
          class="absolute w-full text-sm top-0 grid grid-cols-[50%_50%] gap-1"
        >
          {#each tournament.team?.players ?? [] as player}
            {@const osuUserId = player.osuUserId.toString()}
            <div class="flex items-center gap-2">
              <img
                src={`https://s.ppy.sh/a/${osuUserId}`}
                alt="pfp"
                width={24}
                height={24}
                class="rounded-md w-6 h-6"
              />
              <a href={`https://osu.ppy.sh/u/${osuUserId}`} class="inline-block hover:underline"
                >{player.username}</a
              >
            </div>
          {/each}
      ...
      {:else}
        <div transition:fade={{ duration: 150 }} class="absolute w-full text-sm top-0">
          ...
          {#if tournament.type === 'played'}
            <p>
              {tournament.format} [{tournament.rankRange === 'open'
                ? 'Open Rank'
                : `${tournament.rankRange[0]} - ${tournament.rankRange[1]}`}]
            </p>
            <p class="text-sm flex items-center relative">
              {#if tournament.placement.seed}
                Seed #{tournament.placement.seed.toString()} —
              {/if}
              {tournament.participantCount.toString()}
              {tournament.team ? 'teams' : 'players'}
            </p>
            <span class="font-bold bg-white rounded-md text-black py-1 px-3 absolute top-0 right-0">
              {tournament.placement.final === 1
                ? '1st'
                : tournament.placement.final === 2
                  ? '2nd'
                  : tournament.placement.final === 3
                    ? '3rd'
                    : `Top ${tournament.placement.final}`}
            </span>
          {:else}
            <div class="flex flex-col gap-1 absolute top-0 right-0">
              {#each tournament.roles as role}
                <span class="font-bold bg-white rounded-md text-black py-1 px-3 text-center"
                  >{role}</span
                >
              {/each}
            </div>
    ...
    {#if $showDetails}
      <div
        ...
        class="absolute bottom-0 left-0 p-4 flex flex-col gap-1"
      >
        {#if tournament.type === 'staffed' && tournament.websiteId}
          {@const website =
            websites.find((website) => website.id === tournament.websiteId) ?? websites[0]}
        ...
        {#if tournament.forumPostId && !$showTeams}
          ...
          <div transition:fade={{ duration: 150 }} class="flex text-sm items-center gap-2">
            ...
            <a
              ...
              on:blur={(tournament.type === 'staffed' && tournament.spreadsheetId) ||
              hasTeam ||
              hasWebsite
                ? undefined
                : showDetails.toFalse}>Forum Post</a
            >
          </div>
        {/if}
      </div>
    ...
    {#if $showDetails && (hasTeam || tournament.type === 'staffed')}
      <div transition:fade={{ duration: 300 }} class="absolute bottom-0 right-0 p-4">
        {#if $showViewTooltip}
          <div
            class={`tooltip -top-[26px] z-[2] ${
              tournament.type === 'played'
                ? $showTeams
                  ? '-right-[40px]'
                  : '-left-[3px]'
                : tournament.websiteId
                  ? '-right-[43px]'
                  : '-left-[16px]'
            }`}
        ...
        {#if tournament.type === 'staffed' && tournament.spreadsheetId}
          ...
          <a
            href={spreadsheetUrl}
            target="_blank"
            class="block btn-icon btn-gradient"
            on:mouseenter={showViewTooltip.toTrue}
            on:mouseleave={showViewTooltip.toFalse}
        ...
        {:else if hasTeam || hasWebsite}
          <button
            ...
            on:click={tournament.type === 'played' ? showTeams.toggle : onScreenshotsClickBtn}
            on:mouseenter={showViewTooltip.toTrue}
            on:mouseleave={showViewTooltip.toFalse}
            on:keydown={onScreenshotsKeydown}
            on:blur={showDetails.toFalse}
          >
            {#if tournament.type === 'played'}
              {#if $showTeams}
